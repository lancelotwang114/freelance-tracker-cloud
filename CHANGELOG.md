...
## v3.28.6 — 亮色對比修正 + 暗色主按鈕深字 + 開放捏合縮放（2026-07-16）

### 背景：taste-skill 巡檢，WCAG 相對亮度實算（AA 一般文字需 ≥ 4.5）
暗色主題（E1 暖黑）全數通過；亮色語意色文字大面積不及格。

### 修正 1：亮色 `:root` 四個語意色加深（vs card #fdfcf9）
- `--muted` #8b8677 → **#7a7462**（3.54 → 4.55）— 12px 日期/副標全站受惠
- `--success` #0d9464 → **#0b7a52**（3.76 → 5.22）— 已收款金額
- `--gold` #b8860b → **#8a670a**（3.17 → 5.09）— 「✓ 已收款」標籤
- `--orange` #ea7317 → **#b8520d**（2.94 → 4.82；badge 對 orange-light 2.53 → 4.14）
- danger 4.71 / warning 4.89 / primary 5.33 本來就過，不動
- 這些變數當背景的地方（勾選框、圖表 bar）都是白勾/色塊，加深後對比更好，無回歸

### 修正 2：暗色 `.btn-primary` 白字 → 深字
- 暗色 primary #35b5a8 上白字只有 2.52 → `color: #08211d`（6.68；hover #2a978c 上 4.74）

### 修正 3：viewport 開放捏合縮放
- 移除 `maximum-scale=1.0, user-scalable=no`（WCAG 1.4.4，手機可放大）

### 範圍
純 CSS/HTML 視覺層，無 schema、無同步機制、無 JS 邏輯變動。

---

## v3.28.5 — 總覽「近期案件」→「待完成案件」（2026-07-15）

### 使用者指定 + usage 統計背書
3 天操作統計：熱路徑 = 總覽頁標完成（16 次）。原「近期案件」按日期取最新 10 筆、
含已完成已收款 — 標完後殘留佔位。改成待完成 todo 清單，標完成 row 直接消失。

### 規格
- 篩選：`!done && !cancelled && !isEstimate`（「已收待做」也算待完成）
- 排序：日期舊→新（逾期/最急浮頂；原本新→舊）
- 筆數：全部顯示不截斷（原 10 筆上限拿掉）
- 空態：「☕ 沒有待完成案件」
- 順修：`dashBulkSelectAll` 原本把清單查詢複製一份（slice 10）→ 抽共用 `dashPendingJobs()`，
  render 跟批次全選同源，批次全選=清單全部

### 驗證
同步斷言 7 條全過（13 筆全顯/已完成取消估價不出現/舊→新排序/標題/批次全選 13/
標完成消失 13→12/空態）。純 render 層，無 schema、無同步機制變動。
