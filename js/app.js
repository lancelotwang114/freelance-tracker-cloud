...
async function cloudInitGoogleAuth() {
  ...
  window.addEventListener('online', () => {
    ...
    // v3.25.2（R26）：離線期間 guard 不鎖；上線後給 push/refresh 20 秒自救，
    // 仍是 error（= 不是網路問題，是 auth 死亡）才走鎖定
    setTimeout(() => {
      if (cloudSyncStatus === 'error' && navigator.onLine) showSyncErrorOverlay();
...
function _handleSilentRefreshFailure(errMsg) {
  ...
  if (typeof cloudSetSyncStatus === 'function') {
    cloudSetSyncStatus('error', _isNetworkIssue
      ? '📶 網路不穩，改動已存本機，恢復後自動同步'
      : 'Google 連線過期，請點右上角「重新登入」');
  }
  if (typeof toast === 'function') {
    toast(_isNetworkIssue
      ? '⚠️ 網路不穩，背景會持續重試（改動已存本機）'
      : '⚠️ Google 連線過期，請點頂部紅條重新登入（資料未同步）', 10000);
  }
...
function cloudSignOut() {
  ...
  if (_syncGuardDeferTimer) { clearTimeout(_syncGuardDeferTimer); _syncGuardDeferTimer = null; }
  cloudPushFailRetries = 0;
...
let _syncGuardDeferTimer = null;        // v3.25.2（R27）：modal 開啟時的延後評估 timer
...
function dismissSyncErrorOverlayTemp() {
  ...
  _syncErrorAggressiveRetryTimer = setInterval(() => {
    ...
    // v3.25.2：加 refreshToken（code flow 下 tokenClient 可能沒 ready 但 Worker 續約可用）
    if (cloudAuthState.user && (cloudAuthState.tokenClient || cloudAuthState.refreshToken) && !getValidAccessToken()) {
      _silentRefresh();
    }
    if (cloudPendingChangesCount > 0 && typeof cloudPushNow === 'function') {
...
function showSyncErrorOverlay() {
  ...
  // v3.25.2（R26）：離線不鎖編輯 — 單機離線改動存 localStorage，上線自動推 + mergeStates 兜底，
  //   衝突風險遠小於「整個工具不能用」的成本。全屏鎖只留給線上但 auth 死亡的情境。
  ...
  if (typeof _isAnyModalOpen === 'function' && _isAnyModalOpen()) {
    ...
    console.log('[sync-guard] 編輯 modal 開啟中，延後 15 秒再評估鎖定');
...
async function ensureValidToken(timeoutMs = 30000) {
  ...
  // 沒登入 / 沒有任何續約手段（tokenClient popup 或 refresh token）→ 不能 refresh
  // v3.25.2：加 refreshToken — code flow 下就算 tokenClient 沒 ready 也能走 Worker 續約
  if (!cloudAuthState.user || (!cloudAuthState.tokenClient && !cloudAuthState.refreshToken)) return false;
...
async function driveFetch(url, options = {}) {
  ...
  if (r.status === 401) {
    // v3.25.2（R28）：401 先強制過期本機 token → silent refresh（refresh token 走 Worker）→ 重試一次
    // 「要求使用者重新登入」是最後手段，只留給 refresh 也救不回的情境
    if (!options._retried401) {
      console.warn('[drive-fetch] 401 → 嘗試 silent refresh 後重試一次');
      cloudAuthState.accessToken = null;
      cloudAuthState.tokenExpiresAt = 0;
      const ok = await ensureValidToken();
      if (ok) return driveFetch(url, { ...options, _retried401: true });
    }
    throw new DriveAuthError('Google 連線需要重新整理，請點右上角「重新登入」');
  }
  if (r.status === 403) {
...
function jobRowCompact(j) {
  ...
  return `<div class="row-compact state-${status}" data-job-id="${j.id}" style="border-left:3px solid ${color}"
    ...
    <span class="row-quick-actions">
      ...
      <button onclick="event.stopPropagation(); duplicateJobFromRow('${j.id}')" title="複製為新案件">⧉</button>
    </span>
  </div>`;
}
  ...
  const rows = jobs.map(j => {
    ...
    return `<tr data-job-id="${j.id}"
      ...
      <td class="t-actions" onclick="event.stopPropagation();">
        ...
        <button onclick="duplicateJobFromRow('${j.id}')" title="複製為新案件">⧉</button>
...
// v3.25.2：全部改 null-guard — job-paid / job-paid-at 等舊 id 在付款重構後已移除，
//          原版直接 TypeError（modal 的「複製」按鈕其實一直是壞的），這次連 row ⧉ 一起修
function duplicateJob() {
  ...
  set('job-done', el => { el.checked = false; });
  set('job-paid', el => { el.checked = false; });
  set('job-cancelled', el => { el.checked = false; });
  set('job-done-at', el => { el.value = ''; });
  set('job-paid-at', el => { el.value = ''; });
  // 案件名稱加上「(複製)」字樣，方便辨識
